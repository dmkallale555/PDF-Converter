...
import { getCurrentUser, logoutUser, updateUserProfile, handleFirebaseGoogleAuth } from './utils/authStorage';
import { addConversionRecord } from './utils/historyStorage';
import { auth, onAuthStateChanged, testFirestoreConnection } from './firebase';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (fbUser) => {
      if (fbUser) {
        try {
          const session = await handleFirebaseGoogleAuth(fbUser);
          setUser(session.user);
        } catch (e) {
          console.error('Failed to sync Firebase auth state:', e);
        }
      }
    });
  ...
  const isPdfMergeView = currentRoute === 'pdf-merge';
  ...
  const isPdfToImageView = !isPdfMergeView && !isImageToPdfView && !isImageToImageView && !isBatchView && !isDashboardView && !isHistoryView && !isSettingsView && !isAdminView;

  const getActiveToolTitle = (route: AppRoute) => {
    switch (route) {
      case 'pdf-to-jpg': return 'PDF to JPG Converter';
      case 'pdf-to-png': return 'PDF to PNG Converter';
      case 'pdf-to-webp': return 'PDF to WEBP Converter';
      case 'pdf-to-tiff': return 'PDF to TIFF Converter';
      case 'pdf-to-bmp': return 'PDF to BMP Converter';
      case 'pdf-merge': return 'PDF Merger & Combiner';
      case 'image-to-pdf':
      case 'jpg-to-pdf':
      case 'png-to-pdf': return 'Image to PDF Converter';
      case 'image-to-image':
      case 'jpg-to-png':
      case 'png-to-jpg': return 'Image Format Converter';
      case 'batch-converter': return 'Batch Multi-File Converter';
      case 'dashboard': return 'Member Dashboard';
      case 'history': return 'Conversion History';
      case 'settings': return 'Account Settings';
      case 'admin': return 'Admin Panel';
      default: return 'PDF & Image Conversion Tools';
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] dark:bg-gray-900 flex flex-col text-gray-900 dark:text-gray-100 font-sans antialiased transition-colors">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* ACCESS GATE: If not logged in, require sign up & login to access all tools */}
        {!user ? (
          <AuthGate
            activeToolName={getActiveToolTitle(currentRoute)}
            onOpenAuth={(mode) => setAuthModalMode(mode)}
            onSuccessfulAuth={(session) => setUser(session.user)}
          />
        ) : (
          <>
            ...
            {/* 2. PDF MERGER VIEW */}
            {isPdfMergeView && (
              <PdfMerger onConversionComplete={handleExternalConversionComplete} />
            )}

            {/* 3. IMAGE TO PDF CONVERTER VIEW */}
            {isImageToPdfView && (
              <ImageToPdfConverter
